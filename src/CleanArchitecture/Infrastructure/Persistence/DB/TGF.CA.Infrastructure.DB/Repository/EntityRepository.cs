...
namespace TGF.CA.Infrastructure.DB.Repository
{
    ...
    public abstract class EntityRepository<TRepository, TDbContext, T, TKey>(TDbContext aContext, ILogger<TRepository> aLogger, ISpecificationEvaluator specificationEvaluator)
        : IEntityCommandRepository<T, TKey>, IEntityQueryRepository<T, TKey>, IEntitiyRepository<T, TKey>
        ...
        where TKey : struct, IEquatable<TKey>
    {
        ...
        private readonly InternalEntitiyCommandRepository _commandRepository = new(aContext, aLogger);
        private readonly InternalEntitiyQueryRepository _queryRepository = new(aContext, aLogger, specificationEvaluator);
        ...
        private class InternalEntitiyCommandRepository : EntityCommandRepository<TRepository, TDbContext, T, TKey>
        {
            internal InternalEntitiyCommandRepository(TDbContext aContext, ILogger<TRepository> aLogger) : base(aContext, aLogger)
            {
            }
        }
        private class InternalEntitiyQueryRepository : EntityQueryRepositoryInternal<TRepository, TDbContext, T, TKey>
        {
            internal InternalEntitiyQueryRepository(TDbContext aContext, ILogger<TRepository> aLogger, ISpecificationEvaluator specificationEvaluator) : base(aContext, aLogger, specificationEvaluator)
            {
            }
        }
        #endregion
