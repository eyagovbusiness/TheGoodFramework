...
namespace TGF.CA.Infrastructure.DB.Repository.CQRS.Internal {  
    ...
    where TKey : struct, IEquatable<TKey>
    {
        ...
        => await TryQueryAsync(async cancellationToken => {
            ...
            ? await _specificationEvaluator.GetQuery(Queryable, specification).Where(e => entityIdList.Contains(e.Id)).ToListAsync(cancellationToken)
            : await Queryable.Where(e => entityIdList.Contains(e.Id)).ToListAsync(cancellationToken);
