...
namespace TheGoodFramework.CA.Domain.Primitives.Result
{
    ...
    public class Result<TValue> : Result
    {
        ...
        /// <summary>
        /// Gets a new instance of <see cref="Result{TValueType}"/> from this instance after trying to convert sucessfully this instance <see cref="Result{TValueType}.Value"/> into the target Type 
        /// </summary>
        /// <typeparam name="TConvert">The target Type in which to convert the <see cref="Result{TValueType}.Value"/></typeparam>
        /// <returns></returns>
        public Result<TConvert> GetConverted<TConvert>()
        {
            TConvert lConvertedValue;
            try
            {
                lConvertedValue = (TConvert)Convert.ChangeType(Value, typeof(TConvert));
            }
            catch { return null; }

            return new Result<TConvert>(lConvertedValue, IsSuccess, Error);
        }
    }
}
